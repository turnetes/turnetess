...
class ConfirmacionTurno4 extends StatefulWidget {

  Turno turno;
  //LocalStorage storage;
  ConfirmacionTurno4(this.turno);

  _ConfirmacionTurno4 createState() => _ConfirmacionTurno4(turno);
}

class _ConfirmacionTurno4 extends State<ConfirmacionTurno4> {

  final Turno turno;

  _ConfirmacionTurno4(this.turno);
  @override
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async{
                      ...
                      if (idPropio == turno.idMaquinistaPide ) {
                        ...
                        //Comprobamos que ese maquinista no es candidato a hacer otro turno ese dia..
                        bool? yaEsCandidato = await getCandidatoHaciaTienePeticionTurnoEseDia(idPropio);
                        if(yaEsCandidato!){
                          showAlert3(context,"Ese dia ya has solicitado la peticion de otro turno. Espera a que el compañero te conteste");
                        }
                        else{
                          ...
                          if(haceTurno!){
                            ...
                            //hemos comprbado que no cubre ningun turno a nadie ese dia vamos a ver que no haya otro candidato que se haya colado
                            bool? candidato = await getCandidato();
                            if(candidato!){ //if(candidato != null || candidato!)
                              //ya se ha colado salta showAlert
                              showAlert5(context,"Ya hay otro maquinista que le cubre el turno");
                              Navigator.of(context).pop(true);
                            }
                            else{
                              // todo correcto realiza turno
                              await confirmarCambio(turno,idPropio!);
                              //List<String> listaToken = [];
                              //listaToken.add(snapshot.data!.oneSignalID);
                              //notificacion instantanea
                              //await sendNotification(listaToken,infoTurno(turno),"BUENAS NOTICIAS",3);
                              //notificacion diferida el dia anterior al turno para recordar que hace el turno
                              //le pongo en el campo de firebase del turno mi id de candidato
                              //LocalStorage storage = new LocalStorage('peticiones.json');
                              //candidatoRealizarTurno(turno.idTurno, idPropio!);
  ...
  confirmarCambio(Turno turnoPedido,String idPropio) async{
    showAlert(context,"Confirmado que realizas el turno a tu compañero, revisa la info en tu area de Mis Acuerdos");
  ...
  void sumarTurnoMaquinista(Turno turnoPedido,String idPropio){
    int cont;
